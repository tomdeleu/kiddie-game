...
enum KitchenProps {
    ...
    /// **The door moved to `Props/Doorway.swift`**, because the garden has one
    /// too and `ROOMS.md` §9's three cues are a rule about rooms rather than
    /// about this one. `KitchenRoom` keeps calling it by the name it always
    /// used.
    typealias Doorway = Props.Doorway

    static func doorway(flat: Bool) -> Doorway {
        Props.doorway(flat: flat)
    }
}
