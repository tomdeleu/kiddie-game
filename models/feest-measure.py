"""Frozen lighting ruler for Het Feest's plate-matching hillclimb.

One command, two jobs:

    python3 models/feest-measure.py plate
    blender --background --python models/feest-measure.py -- still NAME.png

The first prints the lighting signature of ``references/feest/roombox.png``.
The second assembles the shipping disco, renders it from the game camera,
...
Signature, on every non-backdrop pixel:
  ...
  open_L     90th-percentile luminance (bright plaster / lit tiles)
...
win that muddies the cream plaster is a revert.
"""
...
PLATE = os.path.join(REPO, "references", "feest", "roombox.png")
STUDY = os.path.join(REPO, "app", "ao-study-feest")
...
def crop_to_room(path, dest):
    ...
    try:
        ...
        cropped = bpy.data.images.new("FeestCrop", x1 - x0, y1 - y0)
...
def _load_scene():
    ...
    path = os.path.join(HERE, "feest-scene.py")
    spec = importlib.util.spec_from_file_location("feest_scene", path)
...
def _load_ao():
    import importlib.util
    path = os.path.join(HERE, "feest-ao.py")
    spec = importlib.util.spec_from_file_location("feest_ao", path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module
...
def calibrate_exposure():
    ...
    plate = signature(PLATE)
    best = None
    for exp in (0.15, 0.25, 0.30, 0.35, 0.40, 0.50):
        ...
        scored = score(row, plate)
        print("e=%.2f  %s  open_err=%.4f  plate_err=%.4f  pass=%s"
              % (exp, format_signature(row), err, scored["plate_err"], scored["pass"]))
        if best is None or scored["plate_err"] < best[0]:
            best = (scored["plate_err"], exp, row, scored)
    print("best exposure %.2f plate_err=%.4f pass=%s"
          % (best[1], best[0], best[3]["pass"]))
    return best
...
def sweep_ambient():
    ...
    print("best ambient %.0f plate_err=%.4f pass=%s" % (best[1], best[0], best[2]["pass"]))
    return best
...
def main():
    ...
    if args[0] == "compare" and len(args) >= 2:
        ...
        return
    raise SystemExit(
        "usage: feest-measure.py [plate|prove|calibrate|baseline|ambient|still NAME.png|compare IMAGE [BASELINE]]")
