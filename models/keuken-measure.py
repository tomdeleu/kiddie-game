"""Frozen lighting ruler for De Keuken's plate-matching hillclimb.

One command, two jobs:

    python3 models/keuken-measure.py plate
    blender --background --python models/keuken-measure.py -- still NAME.png

The first prints the lighting signature of
``references/plates/03-kitchen-roombox.png`` — the plate the owner named.
The second assembles the shipping kitchen, renders it from the game camera,
and prints the same numbers. Changing this file after a baseline is recorded
invalidates every earlier row.
...
PLATE = os.path.join(REPO, "references", "plates", "03-kitchen-roombox.png")
STUDY = os.path.join(REPO, "app", "ao-study-keuken")
...
def _load_scene():
    ...
    path = os.path.join(HERE, "keuken-scene.py")
    spec = importlib.util.spec_from_file_location("keuken_scene", path)
...
def _load_ao():
    import importlib.util
    path = os.path.join(HERE, "keuken-ao.py")
    spec = importlib.util.spec_from_file_location("keuken_ao", path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module
...
def _ao_plane(name, vertices, colour, filename):
    import bpy
    path = os.path.join(
        REPO, "app", "NinaBakeryPOC", "Resources", "Lightmaps", filename)
...
def _add_shell_overlay():
    """The three shipping maps, on the same UV planes KitchenAO uses."""
    half = 0.23
    floor = 0.0042
    wall = 0.23 - 0.012
    height = 0.235
    blush = (0.984, 0.816, 0.792, 1.0)
    cream_light = (0.949, 0.902, 0.863, 1.0)
    cream = (0.894, 0.855, 0.792, 1.0)
    _ao_plane(
        "KeukenFloorAOPreview",
        [(-half, -half, floor), (half, -half, floor),
         (half, half, floor), (-half, half, floor)],
        blush, "KeukenFloorAO.png")
    _ao_plane(
        "KeukenBackWallAOPreview",
        [(-half, wall, 0), (half, wall, 0),
         (half, wall, height), (-half, wall, height)],
        cream_light, "KeukenBackWallAO.png")
    _ao_plane(
        "KeukenLeftWallAOPreview",
        [(-wall, -half, 0), (-wall, half, 0),
         (-wall, half, height), (-wall, -half, height)],
        cream, "KeukenLeftWallAO.png")


def render_shell_overlay():
    """As-it-ships plus the three shipping shell maps, one assemble."""
    scene = _load_scene()
    scene.assemble()
    _add_shell_overlay()
    plate = signature(PLATE)
    still = os.path.join(STUDY, "03-shell-maps.png")
    scene.render_still(still)
    _composite_backdrop(still)
    _score_path(still, plate, "maps")


# Shell and skin stay pale so furniture still reads on the blush floor.
# Everything else is a plate pastel and may be punched.
_PUNCH_SKIP = (
    "Slab", "Floor", "Wall", "NinaSkin", "NinaFace", "Portrait",
)
...
def main():
    ...
    if args[0] == "compare" and len(args) >= 2:
        ...
        return
    raise SystemExit(
        "usage: keuken-measure.py [plate|prove|calibrate|baseline|maps|ambient|punch|still NAME.png|compare IMAGE [BASELINE]]")
